...
--- ############ SUBMODULO DE MANTENIMIENTOS
    ...
    SELECT ROWID,CIP_PERSONA_CODIGO,CIP_GAF_CODIGO,CIP_TIP_CODIGO,CIP_EMP_CODIGO,CIP_AREA_CODIGO,CIP_EST_CODIGO,CIP_GAFETE_FE_INGRESO,CIP_GAFETE_FECHA_EMISION,CIP_GAFETE_OBSERVACION,CIP_GAFETE_FECHA_IMPRESION,CIP_GAFETE_USUARIO,CIP_NUM_REPOSICION,CIP_GAFETE_FOTO FROM CIP_DATOS_GAFETES WHERE (erqfsa) order by cip_persona_codigo

    --// Detalle de Personas

    SELECT ROWID,CIP_PERSONA_CODIGO,CIP_TIP_CODIGO,CIP_EMP_CODIGO,CIP_PUESTO_CODIGO,CIP_DET_PERSONA_FE_ALTA,CIP_DE_PERSONA_DOCTO_ALTA,CIP_DET_PERSONA_FE_BAJA,CIP_DET_PERSONA_DOCTO_BAJA,CIP_DET_PERSONA_COD_ESTATUS,CIP_DET_PERSONA_OBSERVACIONES,CIP_DET_PERSONA_FE_INGRESO,CIP_DET_PERSONA_USUARIO FROM CIP_DET_PERSONAS WHERE (fqweradfz)

    --// Autorizaciones

    SELECT ROWID,CIP_AUT_NUM_REPOSICION,CIP_TIP_CODIGO,CIP_EMP_CODIGO,CIP_PERSONA_CODIGO,CIP_GAF_CODIGO,CIP_AREA_CODIGO,CIP_PER_AUTORIZA_CODIGO,CIP_AUT_FECHA_DEL,CIP_AUT_FECHA_AL,CIP_AUT_COD_ESTATUS,CIP_AUT_POR_MEDIO,CIP_AUT_OBSERVACIONES,CIP_AUT_FE_INGRESO,CIP_AUT_USUARIO,CIP_FIRMA_PORTADOR FROM CIP_AUTORIZACIONES WHERE (rqwerzdfs)

    --// Marbetes

    SELECT ROWID,MAR_NUM_FORMULARIO,MAR_FECHA_FORMULARIO,MAR_NUM_MARBETE,MAR_FECHA_EMISION,MAR_COD_TIP_VEHICULO,MAR_NUM_PLACA,MAR_TARJ_CIRCULA,MAR_NOMBRE_PROPIETARIO,MAR_VEH_PROPIETARIO,MAR_RESPONSABLE,MAR_CODIGO_EMPLEADO,MAR_COD_UNIDAD,MAR_CHASIS,MAR_MODELO,MAR_USO_VEHICULO,MAR_MARCA_VEHICULO,MAR_LINEA_VEHICULO,MAR_COLOR_VEHICULO,MAR_NUM_MOTOR,MAR_NUM_LICEN_RESP,MAR_COD_AUTORIZA,MAR_OBSERVACIONES,MAR_ESTATUS,MAR_USUARIO FROM CIP_MARBETES WHERE (rtqtzsdf)

    --// Ingreso de Vehiculos

    SELECT ROWID,CIP_CODCIA,CIP_EMPRESA,CIP_CODPLC,CIP_NUMPLC,CIP_MARCA,CIP_MODELO,CIP_COLOR,CIP_PUERTAS,CIP_EJES,CIP_MOTOR,CIP_CHASIS,CIP_FLAGAUT,CIP_CODAUT,CIP_FECHA_AUT,CIP_FECHA_ALTA,CIP_USUARIO FROM RHEPQ.CIP_VEHICULOS_AUT WHERE (rqwerzcasd)
...
SELECT ROWID,TDC_CODCIA,TDC_CODIGO,TDC_DESCRIPCION,TDC_CTA_CONTABLE,TDC_CTA_CONTABLE2,TDC_CTA_CONTABLE3,TDC_CTA_PPTO1,TDC_PRIORIDAD,TDC_PRIORIDAD_REPORTE,TDC_NOMBRECORTO,TDC_AFP,TDC_RENTA,TDC_SALARIO_PROM FROM PLA_TDC_TIPO_DESCUENTO WHERE (TDC_CODCIA=:1) and (fasdfas)
   ...
   SELECT ROWID,ISR_TIPO,ISR_DESDE,ISR_HASTA,ISR_VALOR,ISR_PCT,ISR_EXCEDENTE FROM PLA_ISR_ISR WHERE (fasdfa) order by isr_desde
    ...
    SELECT ROWID,PGE_CODCIA,PGE_ANTICIPOS,PGE_ALIMENTAC,PGE_DIAS_INCAP,PGE_PCT_INCAP,PGE_CUOTA_ACEP,PGE_PCT_AGUIN,PGE_PCT_BONIF,PGE_CUOTAS_ANT,PGE_SUBSIDIO_FAM,PGE_MONTO_BECHA,PGE_FACTOR_UTILIDADES,PGE_FACTOR_VENTAJAS,PGE_FACTOR_BONO14,PGE_FACTOR_AGUINALDO,PGE_SEGUMED_EMP,PGE_SEGUMEN_FAM,PGE_SALARIOS_SV,PGE_PCT_SEGURO_VIDA,PGE_FECHA_INI_SV,PGE_FECHA_FIN_SV,PGE_SAL_VIDA_MAX,PGE_SAL_VIDA_MIN,PGE_PORC_SINDICATO_STEPQ,PGE_SINDICATO,PGE_PORC_SINDICATO_UNIDO,PGE_SINDICATO_UNIDO,PGE_PORC_42495,PGE_SALIDA_ADM,PGE_ENTRADA_PLT,PGE_SALIDA_PLT,PGE_PORC_BONO_VACACIONAL,PGE_CUOTA_BONO_VACACIONAL,PGE_PORCEN_TIM_JUNTADIR,PGE_PORCEN_ISR_JUNTADIR,PGE_VALOR_SESION,PGE_BONO_ESCOLAR_HIJOS,PGE_BONO_ESC_EMP_PRIMARIA,PGE_BONO_ESC_EMP_EDUMEDIA,PGE_BONO_ESC_EMP_UNIVERSI,PGE_PORC_PLAN_JUBILACION,PGE_MAX_HRS_TOMADURIA,PGE_ACCESO_TOMADURIA,PGE_GTO_REP_PDTE_JUNTA,PGE_GTO_REP_GTE_GRAL,PGE_GTO_REP_SUBGTE_GRAL,PGE_SINDI_SUTRA,PGE_SIND_STUPEPQPZ,PGE_ORDEN_FESTIVO FROM PLA_PGE_PARAMETROS_GEN WHERE (PGE_CODCIA=:1) and (fasdf)


    --// Parametros fondo vacacion
    SELECT ROWID,FDV_CODCIA,FDV_TIPO,FDV_SAL_INI,FDV_SAL_FIN,FDV_CUOTA_BASE,FDV_PCT,FDV_EXCEDEN,FDV_CUOTA_BASE_PAT,FDV_PCT_PAT,FDV_EXC_PAT FROM PLA_FDV_FONDO_VACACION WHERE (FDV_CODCIA=:1) and (wqafsfa)

    --// Vacaciones y Aguinaldo
    SELECT ROWID,PVA_CODCIA,PVA_TIPO,PVA_CODMIN,PVA_CODMAX,PVA_DIAS_VAC,PVA_DIAS_CSCS,PVA_PCT_VAC,PVA_DIAS_AGU,PVA_PCT_AGU,PVA_DIAS_BON,PVA_PCT_BON FROM PLA_PVA_PARAM_VACACION WHERE (PVA_CODCIA=:1) and (rqweraz)


    --// Seguro Medico Hospitalario
    SELECT ROWID,SMH_CODCIA,SMH_ANIOSERV_INI,SMH_ANIOSERV_FIN,SMH_PCT_CFAM,SMH_PCT_SFAM FROM PLA_SMH_SEGUMED_HOSPI WHERE (SMH_CODCIA=:1) and (rqfasfa)

    --// Tipos de Planilla
    SELECT ROWID,TPL_AREA,TPL_CODCIA,TPL_CODIGO,TPL_DESCRIPCION,TPL_TIPO,TPL_APLICACION,TPL_NO_PERIODO,TPL_TOTAL_PERIODOS FROM PLA_TPL_TIPO_PLANILLA WHERE (TPL_CODCIA=:1) and (gwertafg) order by tpl_codigo,tpl_descripcion

    --// Parametros generacion planilla
    SELECT ROWID,PPL_CODCIA,PPL_CODTPL,PPL_CODPLA,PPL_FECHA_INI,PPL_FECHA_FIN,PPL_FECHA_PAGO,PPL_FRECUENCIA,PPL_MES_CONT,PPL_ESTADO,PPL_NUMERO_PERIODO,PPL_REQUIERE_COMISION,PPL_INCLUYE FROM PLA_PPL_PARAM_PLANI WHERE (PPL_CODCIA=:1) and (erqrfad) order by PPL_CODPLA DESC
    # tipo planilla
    ...
    --// Reporte Plan de Vacaciones # SI

    pr_planv

    --// Vacaciones pendientes # SI

    --// Vacaciones gozadas # SI

    --// asistencia planilla dietas # YA

    --// Actualizacion de Complemento Pacto # N/A SE HACE MANUAL

	--// Actualiza estructura organizatival # YA

    --// Actualiza Paso Salarial # N/A SE REALIZA EN ESTRUCTURA ORGANIZATIVA

    --// Actualiza Seguro Médico # SI

    --// Generación Nomina de Emergencia # Pendiente
    ...
    --- ############ SUBMODULO DE PROCESOS - GENERACION DE NOMINA
...
END;
        ...
        --// Generacion de tabla anterios
            ...
            -- pp_tabla_Bono14 (FORMA)
            -- RHEPQ.SP_ACTUALIZACION_GERENCIA
            -- RHEPQ.SP_ACTUALIZACION_VALORES
            -- RHEPQ.SP_LLENADO_TABLA_UTILIDADES
            -- RHEPQ.SP_REVERSION_TABLA_UTILIDADES
            -- RHEPQ.SP_GENERAR_TABLA_BONO14 (MAIN)

            --************************************
            --** Generar la Tabla de Bono 14 **
            --************************************
            Declare
            ...
            Begin
                            ...
                            Alert_Is := FIND_ALERT('ADVERTENCIA');
                            SET_ALERT_PROPERTY(ALERT_IS, ALERT_MESSAGE_TEXT, 'Este botón es para generar el mes seleccionado en la tabla de BONO 14 si esta seguro de reversar el mes y luego generarlo presione el Botón SI');

                            Alert_Button := Show_Alert(Alert_Is);

                                If Alert_Button = Alert_Button1 Then
                                BEGIN
                                            Reversion_tabla_utilidades;
                                            Llenado_tabla_utilidades;
                                        --	Actualizacion_Gerencias;
                                            --Actualizacion_Valores;
                                            Commit;

                                    :b1.mensaje := 'Proceso Terminado para este mes';
                                    SYNCHRONIZE;
                                EXCEPTION
                                    WHEN NO_DATA_FOUND THEN
                                        :b1.mensaje := 'No encontro datos ';
                                        SYNCHRONIZE;
                                    WHEN TOO_MANY_ROWS THEN
                                        ---:b1.mensaje := 'Muchas Filas ';
                                            SYNCHRONIZE;
                                    WHEN OTHERS THEN
                                        :b1.mensaje := ' oTROS ';
                                        SYNCHRONIZE;
        ...
        --// Generacion tabla actual
            ...
            -- pp_tabla_Bono14_B (FORMA)
            -- SP_GENERAR_TABLA_BONO14_ACTUAL (MAIN)

            -- Reversar tabla de utilidades
            RHEPQ.SP_REVERSION_TABLA_UTILIDADES(pCodCia, pCodTpl, pCodPla, pMes, pFechaDel1, pCursor);

            -- Llenar tabla de utilidades
            RHEPQ.SP_LLENADO_TABLA_UTILIDADES_A(pCodCia, pCodTpl, pCodPla, pMes, pFechaDel, pFechaAl, pFechaDel1, pFechaAl1, pCursor);

            -- Actualización de gerencias
            -- RHEPQ.SP_ACTUALIZACION_GERENCIA(pCodCia, pCodTpl, pCodPla, pMes, pFechaDel, pCursor);

            -- Actualización de valores
            -- RHEPQ.SP_ACTUALIZACION_VALORES(pCodCia, pCodTpl, pCodPla, pMes, pFechaDel, pCursor);
...
GRANT SELECT ON EPQOP.USERS_CONTROLSPWD TO RHEPQ;
GRANT REFERENCES ON EPQOP.USUARIOS_REST TO RHEPQ;
